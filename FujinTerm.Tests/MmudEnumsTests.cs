...
/// <summary>
/// Spot-check tests for <see cref="MmudEnums"/>: every formatter accepts
...
/// </summary>
public sealed class MmudEnumsTests
{
    [Theory]
    [InlineData("0",  "Armour")]
    [InlineData("1",  "Weapon")]
    ...
    public void FormatItemType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatItemType(raw));
    ...
    public void FormatWornSlot_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatWornSlot(raw));
    ...
    public void FormatWeaponType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatWeaponType(raw));
    ...
    public void FormatArmourType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatArmourType(raw));

    [Theory]
    [InlineData("0", "Copper")]
    [InlineData("4", "Runic")]
    public void FormatCurrency_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatCurrency(raw));
    ...
    public void FormatSpellTargets_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatSpellTargets(raw));

    [Theory]
    [InlineData("0", "Cold")]
    [InlineData("6", "Poison")]
    public void FormatSpellAttackType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatSpellAttackType(raw));
    ...
    public void FormatShopType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatShopType(raw));

    [Theory]
    [InlineData("0", "Solo")]
    [InlineData("3", "Stationary")]
    public void FormatMonType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatMonType(raw));
    ...
    public void FormatMonAlignment_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatMonAlignment(raw));
    ...
    public void FormatMagery_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatMagery(raw));
    ...
    public void FormatClassWeaponType_MapsKnown(string raw, string expected)
        => Assert.Equal(expected, MmudEnums.FormatClassWeaponType(raw));

    [Fact]
    public void Format_Unknown_ReturnsUnknownWithCode()
        => Assert.Equal("Unknown(99)", MmudEnums.FormatItemType("99"));
    ...
    public void Format_NonNumeric_ReturnsInputUnchanged(string? raw)
        => Assert.Equal(raw, MmudEnums.FormatItemType(raw));

    [Fact]
    public void FormatAbilityCode_Zero_RendersAsEmpty()
        => Assert.Equal(string.Empty, MmudEnums.FormatAbilityCode("0"));

    [Fact]
    public void FormatAbilityCode_Known_RendersName()
        => Assert.Equal("AC", MmudEnums.FormatAbilityCode("2"));

    [Fact]
    public void FormatAbilityValue_Zero_RendersAsEmpty()
        => Assert.Equal(string.Empty, MmudEnums.FormatAbilityValue("0"));

    [Fact]
    public void FormatAbilityValue_Nonzero_RendersUnchanged()
        => Assert.Equal("15", MmudEnums.FormatAbilityValue("15"));
}
