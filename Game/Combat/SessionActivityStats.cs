...
// RATE-WINDOW figures — kills / experience / currency accrued since the rate
// window last (re)started — by TimeOnline, the elapsed time of that same window.
// Until the Time Analysis section's reset restarts the window, it spans the
// whole session, so the windowed figures equal the totals and the rates read as
// lifetime rates. Resetting Time Analysis re-anchors the window: the totals stay
// put while the rates fall back to zero and climb again from that point.
