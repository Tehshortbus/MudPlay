...
// room-detail popup. Actions and Rooms are null for plain rows. Key/Value are
// named to match the previous KeyValuePair binding so the existing template
// markup keeps working for plain rows.
    ...
    IReadOnlyList<RoomLink>? Rooms = null)
{
    ...
    // The plain-text branch shows only when neither rich branch applies —
    // compiled bindings can't express "!HasActions && !HasRooms" inline.
    public bool IsPlain => !HasActions && !HasRooms;
}
