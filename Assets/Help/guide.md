...
### Cycle Normal and Alternate attack spells

**Default:** Off
**What it does:** Off (default): Alternate is a one-way fallback, exactly as described above — once it takes over for a target, MudPlay never switches back to Normal even if it becomes affordable again. On: MudPlay keeps re-checking BOTH spells every round instead of committing. Whichever one is currently firing keeps firing until *it* stops being castable (its own Max casts / Min mana / an immunity), at which point MudPlay swaps to the other — including switching back to Normal once it's castable again (e.g. your mana regenerates back above its reserve). A fresh target always reopens on Normal first.
**When you might turn it on:** You've got a strong, mana-hungry Normal spell and a cheaper Alternate — cycling spends the strong one whenever you can afford it and fills the gaps with the cheap one instead of parking on the cheap one (or the weapon) for the rest of the fight.
**Important notes:** Has no effect unless both Normal and Alternate attack spell rows are configured. A per-monster attack-spell override (Game Data) still takes priority over cycling for that species.
