...
The **Overlays ▾** button layers lairs, shops, spell rooms, **level-blocked** rooms, and **level gates** onto the map and toggles the **Legend** — which you can **drag anywhere on the map** (it remembers where you put it; toggle it off and back on and it snaps back into view if the window has since shrunk). The Legend keys every room-cell marker the map draws: the amber-ringed **current room**, the blue-ringed **walk-to destination**, room fills (lair, shop/bank, spell, auto-lair, up/down/up+down exit rooms), and the overlay glyphs — **deathpile** skull, **boss** crown (with a red halt ring when it's a *stop-before* boss), **trainer** chevrons, **gang-house** robot, **avoid** (red X), **stash** (gold X), and the fading green **@where** result. Route lines are colour-coded — walk-to **blue**, a running loop **green**, a loop you're previewing **red**, an Auto-Lair approach **orange** (these four are recolourable under Settings → General, so they're described here rather than pinned in the Legend). Exit stubs carry their own colours (shown in the Legend): **red** for a trapped exit, **magenta** "Action required" for an exit you can't just walk — one that needs a command or in-room action to cross (a `go path`-style named exit, a lever, or an ask-a-guard door), and **cyan** for a hidden exit revealed with `sea`.

**Level-blocked** (off by default) paints every room your **current level** shuts you out of in translucent red, over whatever the room already is. It's not just the gated doorway: a level gate seals everything that can only be reached *through* it, so the whole pocket behind it lights up — Newhaven's Arena and the Dungeon Entrance and Small Cavern beyond it are all one blocked area to an over-level character. The game tests the gate when you **enter** a room, so standing inside a pocket you've outgrown shows nothing blocked (the way out isn't gated), and the picture updates when you move or level up. Rooms that are simply unreachable for other reasons — a disconnected area, a teleport-only pocket — are never painted; only rooms you could reach if not for your level. Your choice is saved per character.

The overlay judges reachability with the **same movement filter the router plans routes with**, so it can't disagree with the route picker about whether somewhere is reachable. Every obstacle other than your level — doors, locks, keys, tolls, class and alignment windows, hazards — is applied to *both* halves of that judgement, so it cancels out: a locked door can never paint a room red, and can never stop one being painted either. **Only your level puts a room in the overlay.** What that buys you is honesty about the alternatives: an area sealed behind a level gate stays painted even when the map data appears to offer a second way in through a door nobody can actually open.

**Level gates** (off by default) answers the other half of the question: *where is the gate?* It puts a small **amber wedge in the top-left corner** of every room that holds a level gate you can't pass — a room you can still walk into and stand in, whose way onward is shut to you. That's deliberately a different mark from the **red exit stubs**, which mean a trap: a level gate is a locked door, not a hazard. It covers both level-gated exits and level-restricted boat sailings, so a dock whose captain won't take you until level 50 is marked too. Because a gate belongs to the room rather than to where you're standing, this overlay looks the same wherever you are, and updates when you level up. Your choice is saved per character.

The two overlays are complements, and they're most useful together: **Level-blocked** shows the territory you're shut out of, **Level gates** shows the doors doing the shutting. A gate room never appears in the blocked set — you *can* reach it, it's the far side that's sealed — so with only the blocked overlay lit, the doorway itself stays unmarked. Note that **Level gates considers nothing but your level**: a room walled off by a lock nobody can pick, an item you don't have, or a strength requirement you can't meet is not a level gate and is not marked here.

Traps are **directional**, so a connecting line is only red on the trapped side: a line that's red for its **whole** length is trapped **both** ways, while one red for **half** its length (the half against the room whose exit is trapped) is a **one-way** trap — safe to walk back the other direction.
