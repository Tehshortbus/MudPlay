...
- **Versioning (semver, post-1.0)**: the version lives once in `FujinTerm.csproj` `<Version>` (`AppInfo.Version` reads it back for the `@version` reply). Bump it with the change — **MAJOR** = whole-program refactor, **MINOR** = a large PR, **PATCH** = a small / bugfix PR. **The PATCH increments by the number of bug reports handled** — one report on 1.5.11 lands 1.5.12; a batch of five handed over together advances 1.5.11 → 1.5.16. The version counts the reports, but the changelog does **not** need one entry per report: a batch shares a single consolidated `## <final-version>` entry whose bullets cover all the fixes.
