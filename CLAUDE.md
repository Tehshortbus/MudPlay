...
- **Versioning (semver, post-1.0)**: the version lives once in `FujinTerm.csproj` `<Version>` (`AppInfo.Version` reads it back for the `@version` reply). Bump it with the change — **MAJOR** = whole-program refactor, **MINOR** = a large PR, **PATCH** = a small / bugfix PR. **Each bug report handled bumps the PATCH by exactly one** — a single report on 1.5.11 lands 1.5.12; a batch of five reports advances 1.5.11 → 1.5.16 (one PATCH step + one changelog entry per report, even when several ride the same PR).
