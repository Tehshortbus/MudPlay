...
> - Fixed combat stalling for ~5s after a kill whose death routed through the exp+Off fallback (no specific death-line pattern): the engine's own independent kill-inference reprocessed the same kill a second time and incorrectly dropped the fresh survivor the fallback path had already re-engaged
>
