...
> **Version 1.5.0** — A readability batch across Session Stats, navigation, and the Player Workshop quest view, all from live play. **Changed:** Session Stats currency now reads as coin denominations instead of a comma-grouped copper count — 1000 copper an hour shows as `10 gold/hr`, not `1,000/hr`, with the full itemised wealth line (`1 runic 93 platinum 5 gold 6 copper`) moved to a hover tooltip; the exp/hr and kills/hr graphs now label their current rate (`5.7k`) in the header so you can read it at a glance; the navigation top-bar label shows step progress and the lap number while looping (`step 3/8 · lap 2`); the main-window looping chip is trimmed to Looping + lap + XP/hr; and multi-class ability quests (Smash, Meditate, Supernatural Stealth) show each class's own required level in the quest "Requires" line (`Warrior-22, Mystic-20`) instead of bare class names. **Fixed:** the rate graphs plotted a rolling-window figure that didn't match the table's session-lifetime stat — they now share the same cumulative-average basis, so the graph's right edge equals the headline number; and the navigation lap counter, which used to freeze at lap 1, now counts every lap and resets when the loop stops. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
