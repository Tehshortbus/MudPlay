...
> **Version 1.5.8** — Walk-to no longer wedges into a "walking but not moving" state after crossing into a new area. **Fixed:** every move the walker sends echoes back over the wire, and the room tracker debounced that echo on a fixed 100 ms window. Entering a new area triggers a synchronous map re-root that runs between the walker announcing a move and its echo arriving — pushing the echo past 100 ms, so it was treated as a phantom second step and jammed the pending-move queue (the tracker then held `Pending` forever and the walker never advanced; the brief "map flip" was that same re-root). Stopping and re-queuing inherited the jammed queue. The echo is now matched by a **consume-once claim** — armed only by the engine's own send, cleared by the first matching echo, timing-independent — so a re-root of any duration can't turn it into a phantom. A generous expiry bounds a claim whose echo never comes, and manual movement (which never arms a claim) is unaffected. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
