...
> **Version 1.5.5** — Your own character being dropped (mortally wounded at 0 HP or below) is now handled cleanly. **Fixed:** background engines kept firing commands the game rejects with `You may not do that while you are mortally wounded!` while you were down — dropping now raises a blanket engine-send hold and a `MortallyWounded` movement pause until you recover (the emergency low-HP hangup stays exempt as your last escape). A self-drop also removes you from the party game-side, so the client now clears the stale roster / leader / following flags instead of holding movement on a party you've left; recovery re-confirms membership only from a real follow / `par` signal (a leader re-invite is required to rejoin). See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
