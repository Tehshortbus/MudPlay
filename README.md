...
> **Version 1.2.3** — Corrects how damage-type resistance splits into three unlike flavors. `GAME_MECHANICS.md` no longer lumps every `Resist-*` code under one rule: only the five *elemental* types (Cold/Fire/Stone/Lightning/Water, `AttType` 0/1/2/3/5) follow the flat "100% = 0 damage, over-100% heals" curve that a pre-emptive skip can rely on. **Magic Resist** (code 36, on `AttType 4` "Normal" spells) is a *probabilistic* cut where 100 is not 0 damage, and **poison** (`AttType 6`) is *not resistible at all* — a binary immunity sourced from race/items (Kang race, golden headdress, swamp/snakeskin boots). Adds the `AttType`→element→resist-code mapping table. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
