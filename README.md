...
> **Version 1.2.2** — Records how attack spells actually fail to damage a monster. `GAME_MECHANICS.md` now separates three independent, 1.11p-confirmed failure modes: `SpellImmu +N` (a level gate blocking any spell learnable below level N), a spell targeting restriction such as the priest `harm` spell's *living-only* tag that no `NonLiving` monster can be hurt by (the real cause of `Your spell has no effect on <monster>.`, corrected from the earlier "damage-type immunity" framing), and flat percentage resistance per damage type (`Resist-Fire +50` halves fire damage; 100% deals 0, over-100% heals). See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
