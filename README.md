...
> **Version 1.4.8** — A crash fix: the navigation self-healing recovery could take the whole client down when it gave up on a lost route. **Fixed:** the client crashed to desktop when navigation recovery failed terminally (e.g. a "go path" that got lost with no anchor to backtrack from). The recovery gate's failure path called the engine's abort — which resets the engine and detaches it from the gate, nulling the gate's engine reference — then the next line read that now-null reference to name the engine in the failure event, throwing a `NullReferenceException` that aborted the process. The gate now captures the engine name *before* the re-entrant abort call, so a failed recovery surfaces the normal "Lost — use the map to set your location" dialog instead of killing the app. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
