...
> **Version 1.2.3** — Corrects how damage-type resistance splits into three unlike flavors, and records the spell-targeting monster-type taxonomy. `GAME_MECHANICS.md` no longer lumps every `Resist-*` code under one rule: only the five *elemental* types (Cold/Fire/Stone/Lightning/Water, `AttType` 0/1/2/3/5) follow the flat "100% = 0 damage, over-100% heals" curve a pre-emptive skip can rely on. **Magic Resist** (code 36, on `AttType 4` "Normal" spells) is a *two-part* cut — a partial reduction capped at 50% (75% under AntiMagic) plus a separate `M.R./2`% full-resist roll — so 100 M.R. is never 0 damage; and **poison** (`AttType 6`) is *not resistible at all* — a binary immunity sourced from race/items (Kang race, golden headdress, swamp/snakeskin boots). A new *spell targeting* section records how a spell's targeting tag (`AffectsLivingOnly`/`AffectsUndeadOnly`/`AffectsAnimalsOnly`/none) matches a monster's type flags — including the dedicated `Undead` **byte-boolean** column, which must be tested `!= 0` (values `0`/`1`/`255` all appear in 1.11p), never `== 1`. Adds the `AttType`→element→resist-code mapping table and the `TypeOfResists` roll-eligibility flag. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
