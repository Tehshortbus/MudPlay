...
> **Version 1.4.2** — Two death-handling fixes for automated routes. **Added:** (1) Your own death now halts movement until you manually resume — the client recognises the canonical *"You have been slain by …"* line and asserts the same pause the manual **Pause** button uses, holding you in the graveyard you respawn into (stripped of non-loyal gear, teleported in alone, your party disbanded if you led it) until you deliberately resume from the Navigation window; previously an active loop would just walk your freshly-revived character back into whatever killed it. While the hold is active the Navigation chip reads *Paused — recovering*. (2) When a party member dies mid-route while you're leading, the client now clears their corpse's phantom invite slot so your loop keeps running — a dead non-leader lingers in `par` as an `[Invited]` slot indistinguishable from a real recruit, which would otherwise stall your loop on the *If leading, wait only* window; because every observer sees a *"&lt;Name&gt; has died."* line, the client waits for the fight to finish, confirms the slot, and `uninvite`s it. Every action is doubly bounded — only a name that was an *active* member at death, and only once it actually shows invited — so a pending recruit or same-named mob can never trigger it. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
