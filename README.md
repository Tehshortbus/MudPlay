...
> **Version 1.4.10** — The Health tab's "Hang up if below" ticker can now go negative, down to the realm death floor, so you can set the emergency disconnect deep in the bleeding-out band, closer to death (issue #107). **Changed:** hitting 0 HP in MajorMUD only *drops* you — you're bleeding out but still revivable and still able to hang up — and death only happens at the realm's negative floor (default -25); HP% goes negative too, exactly as the `par` display shows. The ticker used to floor at 0, so your only options were "hang the instant I drop" or a positive-HP panic button. It's now one continuous scale from 100 %/max down through 0 into the negatives, bottoming out at the death floor — set the hangup anywhere on it (e.g. hang at -15 HP, or the percentage that resolves there), in **Percentage** or **Value** mode. Sliding it to the floor is the natural "never hang up" position; **0 is a live trigger** now, not a disable — to turn the auto-hangup off entirely, use the **Disable hang-ups** master switch. The ticker's lower bound and the engine's fire window read the same active-BBS death floor, so the UI can never offer a value the engine would reject. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
