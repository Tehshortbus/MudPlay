...
> **Version 1.4.4** — Four fixes from live play: combat follow-up attacks, room tracking on login, the loop-save prompt, and the navigation activity chip. **Fixed:** with *"attack after last party member"* on, your follow-up swing never fired for a member who showed a family name — MajorMUD announces actions by the *given* name alone while the `par` roster stores the full *"Given Family"* name, so the two never matched (the same mismatch broke target-priority follow); matching now normalises both to the given name. On login your location came up *"completely unknown"* even while standing in a known room, because the realm-entry keystroke (default `E`, which collides with cardinal *East*) was sniffed as an East step and walked the freshly-hydrated tracker off the login room; that keystroke is now flagged as a menu selection, not a move. Saving a running loop falsely prompted *"you changed the loop — run the new one?"* when nothing was edited — the runner rotates its waypoints in place as it walks, so an untouched loop read as changed; the prompt now compares against what you actually opened. And the Navigation activity chip claimed *"looping and moving"* while the character stood still, because a failed loop raised its event before resetting state; the runner now resets to Idle first. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
