...
> **Version 1.4.8** — Three navigation fixes from live play. **Fixed:** (1) the client crashed to desktop when navigation recovery failed terminally (e.g. a "go path" that got lost with no anchor to backtrack from) — the recovery gate's failure path aborted the engine, which detaches it from the gate and nulls the gate's engine reference, then the next line read that now-null reference to name the engine, throwing a `NullReferenceException` that aborted the process; the gate now captures the engine name *before* the re-entrant abort. (2) A stationary player parked in an ambiguous area (identical "Main Road" rooms, a "Darkwood Forest" tile) could watch their map marker vanish, because every passive redisplay of the same room — an Enter echo, a cash-on-ground notice, a party arrival — accrued a "suspect strike" until the third declared them Lost; a redisplay with no move since the last one now carries no new evidence and no longer strikes. (3) A `go path` move inside a same-name area could get the tracker completely lost — the hidden text exit left the redisplay showing only visible exits and the manual move recorded no cardinal, so prediction was skipped and a name+exits search couldn't match; the tracker now follows the deterministic go-path graph edge directly, in both the pending-move check and replay recovery. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
