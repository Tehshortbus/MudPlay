...
> **Version 1.4.1** — A live activity-status chip in the Navigation top bar, so a stalled loop explains itself. **Added:** next to the loop name and step counter, a colour-coded status chip that reads the movement-engine's pause gates directly — *Moving* while stepping, *Fighting* while a room is held for combat, *Waiting — …* with the actual reason (resting on low HP, meditating on low mana, a hurt party member, a pending invitee, following the leader, corpse recovery, looting), or *Paused* when you paused it yourself. Previously a loop that stopped mid-run gave no on-screen reason; the chip now answers "why did my loop just stop?" at a glance (backed by a new fine-grained `GatesChanged` signal so the reason stays accurate even when one hold swaps straight into another). See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
