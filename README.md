...
> **Version 1.3.0** — Adds a pre-emptive elemental-resistance guard for attack spells. Before casting a configured Normal / Alternate attack spell, the combat engine checks the spell's damage element (`AttType`) against the monster's `Resist-<type>` value and, when the target resists that element **≥ 100%** (where the spell deals 0 damage or *heals* the monster), skips the slot down the cascade (Normal → Alternate → weapon) — never wasting a round or its mana. It's deterministic, so only the five *elemental* types are guarded: **Magic Resist** (`AttType 4`, capped & probabilistic) and **poison** (`AttType 6`, a binary immunity) are never pre-empted, and a negative or partial (1–99%) resist still fires. Two new fail-open game-data indexes (`MonsterResistIndex`, `SpellAttackTypeIndex`) back it, and `GAME_MECHANICS.md` now records that elemental resist is *signed* (negative = vulnerability = extra damage). Also fixes undead monsters whose `Undead` flag was stored as `255` being shown as *not* undead in the Game Data browser. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
