...
> **Version 1.4.2** — When you die, movement now stops and stays stopped until you resume it yourself — so a running loop, walk-to, or Auto-Lair can't march you straight back out of the graveyard before you've recovered. **Added:** the client now recognises your own death (the canonical *"You have been slain by …"* line) and asserts the same pause the manual **Pause** button uses, halting every movement engine and holding you in the graveyard you respawn into until you deliberately resume from the Navigation window. Dying drops your non-loyal gear and teleports you in alone, and a leader's party disbands on death — so afterwards you're always the one who'd drive movement, and previously an active loop would just walk your stripped, freshly-revived character back into whatever killed it. Because the hold rides the existing user-pause, every resume affordance already clears it and it can never leave a stuck gate; while it's active the Navigation activity chip reads *Paused — recovering* so it's clear why you're stopped. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
