...
> **Version 1.5.10** — When you're leading a party, the navigator can now route around a toll gate a follower can't afford, instead of walking the group to a gate one member gets stranded at. **Added:** a `(Toll: N)` map exit is per-crosser — every member needs `N × 100` copper on hand or they're left behind. The navigator previously only weighed the *leader's* own wallet against a toll, so it could march the whole party through and strand a broke follower. With the new **Settings → Other → "Avoid party-unaffordable tolls"** toggle on, it folds every member's on-hand wealth into the decision and routes around any toll a member can't cover. Because wealth drifts constantly with loot and spending, it's **demand-polled** — the client only sends the party `@wealth` while actually evaluating a toll on a candidate route, never on a timer. The first pass conservatively avoids the toll while the readings warm up, and the next lap uses the fresh figures; a member who hasn't reported is treated as unable to pay, so nobody gets marched to a gate they can't clear. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
