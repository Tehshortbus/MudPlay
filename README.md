...
> **Version 1.5.6** — A party healer now rescues a downed ally on its own. **Fixed:** seeing `<name> drops to the ground!` used to leave a healer idle while the ally stayed at 0 HP — the client now treats an ally's drop as a wait condition, pausing movement (an `AllyDown` hold), sending `aid <name>`, and healing them by name through the normal spell tiers even though a dropped ally has left the `par` roster, polling their health with an `@health` telepath until they recover. It then releases the hold and, if you're leading, re-invites them (recovery restores their ability to act but not their party membership). The hardest case — the ally who dropped was the **leader**, whose disconnect had already dissolved the party — is recognised too, via a short recent-leader memory, so a leader's drop gets aided and held rather than ignored. See the [version history](CHANGELOG.md) for the full changelog.
<!-- current-version:end -->
